"""Python Driver for NewFocus controllers
    ...
    __git__ = "https://github.com/bdhammel/python_newport_controller"
Requirements:
    python (version > 2.7)
    re
...
class Controller(object):
    ...
    def __init__(self, idProduct, idVendor, logger=None):
    ...
    def _connect(self):
        ...
        self.debug("Connected to Motor Controller Model {}. Firmware {} {} {}\n".format(
            *resp.split(' ')
        ))
        for m in range(1, 5):
            resp = self.command("{}QM?".format(m))
            self.debug("Motor #{motor_number}: {status}".format(
                motor_number=m,
    ...
    def parse_command(self, newfocus_command):
        ...
        if m:
            ...
            self.error("ERROR! Command {} was not a valid format".format(
                newfocus_command
            ))
    ...
    def command(self, newfocus_command):
        ...
        # the driver
        self.debug("1: {}".format(newfocus_command.strip()))
        self.debug("2: {}".format(newfocus_command))
...
if __name__ == '__main__':
    ...
    if not (idProduct or idVendor):
        print('Run the following comman0x4000d in a new terminal window:')
...
# TODO: rewrite code for specific TangoServer implementation - compact, implement logger (+)